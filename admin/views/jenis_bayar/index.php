          ...
          <table class="w-full ">
            <thead>
              <tr class="text-sm border-2 font-bold">
                ...
                <td class="px-5">Nama Jenis</td> 
                <td class="text-center" style="width: 100px;">Aksi</td>
              </tr>
            </thead>
            <tbody>
              <?php $no = 1;
                if (isset($data['jenis_bayar']) && count($data['jenis_bayar']) > 0) {
                  foreach ($data['jenis_bayar'] as $row) {
              ?>
                <tr class="text-xs p-5">
                  <td class="py-2 text-center" style="width: 40px;"><?php echo $no;?></td>
                  <td class="py-2 px-5"><?php echo $row['nama_jenis']; ?></td>
                  <td class="py-2" style="width: 100px;">
                    <div class="w-full flex justify-center items-start">
                      <div class="w-1/3 px-1">
                        <a href="form.php?aksi=edit&id=<?php echo $row['id_jenis']?>" class="px-2 py-1 bg-blue-400 rounded">
                          <i class="fa fa-pencil"></i>
                        </a>
                      </div>
                    
                      <div class="w-1/3 px-1">
                        <a href="?aksi=delete&id=<?php echo $row['id_jenis'];?>" class="px-2 py-1 bg-red-600 rounded">
              ...
              <?php $no++ ; } } ?>
              <?php if (isset($data['jenis_bayar']) && count($data['jenis_bayar']) < 1) { ?>
